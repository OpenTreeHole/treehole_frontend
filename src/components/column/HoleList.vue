...
import { getHole, listHoles } from '@/apis'
...
export default class HoleList extends BaseComponentOrView {
  ...
  debouncedCalculateLines: DebouncedFunc<() => void>
  lineHeight = 10
  ...
  blockedTagsChanged(newVal: ITag[], oldVal: ITag[]) {
    if (newVal.every((tag) => oldVal.find((oldTag) => oldTag.name === tag.name)))
      this.holes.filter((v) => v.tags.every((tag) => !this.blockedTags.find((blockedTag) => blockedTag.name === tag.name)))
  ...
  holesChanged() {
    setTimeout(() => {
      ...
      this.lineHeight = element ? parseInt(window.getComputedStyle(element, null).getPropertyValue('line-height')) : 10
      this.calculateLines()
    }, 100)
  }
