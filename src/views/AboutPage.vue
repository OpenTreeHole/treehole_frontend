<template>
  <v-container>
    ...
    <v-card>
      ...
      <v-card-text v-if="!installed"> 您正在使用网页版 FDU Hole，请<span>安装本应用</span>以获得更好的体验！ </v-card-text>
    ...
    <v-card>
      ...
      <v-card-text>
        <ul>
          <li
            v-for="(item, i) in $feConfig.friendLinks"
            :key="i"
          >
...
export default class AboutPage extends BaseView {
  ...
  updateMsg = ['您正在使用开发版，如遇 BUG 请前往 GitHub 提交 Issue 或 Pull Request！', '检测到新版本，重载应用以升级！', '您正在使用最新版的FDU Hole！']
  ...
  updateUpdateMsgIndex(): number {
    ...
    if (latestVersion[0] < currentVersion[0] || latestVersion[1] < currentVersion[1] || latestVersion[2] < currentVersion[2]) {
      return 0 // 正在使用开发板
    } else if (latestVersion[0] > currentVersion[0] || latestVersion[1] > currentVersion[1] || latestVersion[2] > currentVersion[2]) {
      return 1 // 有新版
    } else {
      return 2 // 正在使用最新发布版
    }
